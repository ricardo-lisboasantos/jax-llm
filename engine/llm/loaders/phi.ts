...
import type { PhiModel } from "../phi.ts";

const mapper = new WeightMapper({
  prefix: { "model.": "" },
  substring: {
    embed_tokens: "embedTokens",
    final_layernorm: "finalLayernorm",
...
export function phiFromSafetensors(
  ...
  const model = safetensors.toNested(hydrated) as PhiModel;
  if (model.layers.length !== PHI_CONFIG.numHiddenLayers) {
    throw new Error(
      `Expected ${PHI_CONFIG.numHiddenLayers} Phi-2 layers, found ${model.layers.length}`,
    );
  }
